...
### Changes
- chore(release): prepare 0.5.0 (2e639e0)
- test(release): align version and clippy assertions (a06044a)
- feat(ui): hide device firmware extraction entry (f6d09b6)
- style(frontend): apply updated formatting (3f49f2d)
- refactor(sector-ui): remove unused conversion types (0ee4d75)
- test(firmware-ui): cover flash mode labels (7bdbc4c)
- fix(theme): correct preview info content (9adf732)
- test(taskbar): cover event decision boundaries (8e7dd79)
- test(packer): cover command error boundaries (314137f)
- test(flash): cover transfer content boundaries (8888b5d)
- fix(flash): enforce session device invariant (a9ac8c2)
- test(mass): cover state and event contracts (ec6c891)
- test(efex): cover function device boundaries (d8d4396)
- test(frontend): cover page and sector composition (a1cd646)
- test(flash): cover cancellation command boundaries (c693b67)
- test(efex): cover session and timeout commands (7049ff6)
- fix(adb): report poisoned selection state (13fd6f7)
- test(frontend): cover editor and content boundaries (89ad705)
- test(firmware): cover IPC command adapters (3e1cdad)
- test(dtb): cover IPC command boundaries (2281668)
- fix(gpio-ui): cancel stale device restoration (fbea251)
- fix(adb-ui): clean up late drag registrations (a0c5b0d)
- test(frontend): cover top-level workflow composition (3810492)
- test(frontend): raise global coverage gates (ed95730)
- fix(logging): enforce component log limits (29bb720)
- fix(firmware-loader): harden content boundaries (ff59c87)
- fix(partition-ui): validate editor inputs and progress (bafd164)
- fix(packer-ui): validate converter numeric boundaries (ed27425)
- fix(i18n): add validation and close labels (e681e5c)
- test(settings): cover persistence and update boundaries (49d4fa8)
- test(adb-ui): cover explorer content and editor races (2b22ac4)
- test(i18n): recognize empty-view architecture keys (beeba01)
- fix(gpio-ui): guard progress and label icon actions (3e5089f)
- fix(mass-production): clamp slot progress display (0d7d693)
- fix(generic-flash): validate manual offset input (fa95c4f)
- test(downloader): cover component states and content (78c5831)
- fix(efel-ui): preserve selected architecture in empty view (a7ee6ea)
- test(extraction): cover component content and boundaries (6c68712)
- test(i18n): enforce locale content contracts (b10fe4f)
- test(frontend): cover shared UI and GPIO hooks (e584a7f)
- fix(frontend): stabilize firmware workflows (fd0d0fb)
- fix(frontend): harden device and EFEL workflows (17495c0)
- test(frontend): cover shared services and contracts (ac8fcf4)
- test(rust): cover service state and IPC contracts (b1ab630)
- fix(packer): guard image layout and allocation ranges (d40aad3)
- test(rust): harden firmware and disk parsers (6472d96)
- fix(flash): validate sparse image boundaries (c6ac63f)
- fix(efex): scale transfer timeouts by payload size (66d8203)
- test: add project-wide quality gates (48498dc)
- fix(build): align TypeScript and Vite module imports (1175c32)
- chore(deps): refresh frontend and Rust dependencies (6e925dc)
- feat(firmware): add Device Firmware Extraction (flash dump) module (3ea2dd7)
- chore(libefex): update subproject commit reference (fccf8c6)
- style(firmware): collapse long type-module use blocks (c56d8ae)
- feat(firmware): resolve board config from FES DTB when sys_config is empty (29d956e)
